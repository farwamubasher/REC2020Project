...
movies_data  = pandas.read_csv(r'dataset\tmdb_5000_movies.csv')
credit_data  = pandas.read_csv(r'dataset\tmdb_5000_credits.csv')
    ...
    if (menu == 1):
        ...
        temp_df = movies_data.loc[movies_data['original_language'] == ln, ['original_language', 'title', 'vote_average','popularity','homepage']] #locate rows where genre value matches genre value inputted by user, grab genre, name, popularity
        ...
        print("Please select a genre you prefer\n Action - type 1\nAdventure - type 2\nFantasy - type 3\nScience Fiction - type 4\nCrime - type 5\nDrama - type 6\nThriller - type 7\nAnimation - type 8\nFamily - type 9\nWestern - type 10\nComedy - type 11\nRomance - type 12\nHorror - type 13\nMystery - type 14\nWar - type 15\nHistory - type 16\nMusic - type 17\nDocumentary - type 18\nForeign - type 19\nTV Movie - type 20\n")
        check = input("Input:")
        if (check == '1'):
            genre = 'Action'
        if (check == '2'):
        ...
        genre_df['genres'] = [genre['name'] for genre in genre_df['genres']]  #replace each genre value in row with the name value in the dict
        temp_df = genre_df.loc[genre_df['genres'] == genre, ['genres', 'title', 'vote_average','popularity','homepage']] #locate rows where genre value matches genre value inputted by user, grab genre, name, popularity
        temp_df = temp_df.sort_values(by='vote_average', ascending=False)  #sort by popularity descending
        print(temp_df.head()) #print first 5 rows
        ...
        temp_df = keyword_df.loc[keyword_df['keywords'] == keyword, ['keywords', 'title', 'vote_average','popularity','homepage']]
        ...
        temp_df = movies_data.loc[movies_data['runtime'] < time , ['runtime', 'title', 'vote_average','popularity','homepage']] #locate rows where genre value matches genre value inputted by user, grab genre, name, popularity
        temp_df = temp_df[movies_data['runtime'] != 0].sort_values(by='vote_average', ascending=False)  #sort by popularity descending
        print(temp_df.head()) #print first 5 rows
